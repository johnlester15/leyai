...
export default function Settings({
  ...
  useEffect(() => {
    setMounted(true);
    try {
      const saved = localStorage.getItem("leyani_settings");
      if (saved) {
  ...
  useEffect(() => {
    if (!mounted) return;
    try {
      localStorage.setItem("leyani_settings", JSON.stringify(settings));
    } catch {}
  }, [settings, mounted]);
    ...
    <section className="bg-[#232323] border border-[#2e2e2e] rounded-2xl p-6 shadow-2xl hover:shadow-[0_8px_32px_rgba(62,207,142,0.08)] transition-shadow duration-300">
      ...
      <div className="grid grid-cols-3 gap-3 mb-5">
        <div>
          ...
          <select
            className="w-full bg-[#1c1c1c] border border-[#2e2e2e] rounded-lg px-2 py-2 text-xs outline-none focus:border-[#3ecf8e]"
            aria-label="Quiz type"
            value={settings.type}
            onChange={(e) => setSettings({ ...settings, type: e.target.value as "Mixed" | "MCQ" | "Identification" })}>
            <option>Mixed</option>
            <option>MCQ</option>
